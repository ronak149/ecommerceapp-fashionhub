...
const ProductListScreen = () => {
    ...
    const [{ products, loading, error, loadingCreate, loadingDelete, successDelete }, dispatch] = useReducer(reducer, {
        products: [],
        loading: false,
        error: '',
        successDelete: false,
    ...
    const createProductHandler = async () => {
        if (window.confirm('New Product will be created')) {
            try {
                ...
                navigate(`/search/products/${data.product.slug}`);
            }
            catch (err) {
    ...
    const deleteProductHandler = async (product) => {
        if (window.confirm('WARNING: Product will be deleted !')) {
            try {
            ...
            <div className="row m-2 mt-0  g-2 shadow">
                ...
                <div class="col-md-10 p-3">
                    ...
                    <form className="row mb-3 text-center" role="search">
                        <div className="col-6"><input className="form-control" type="search" placeholder="Search" aria-label="Search" onChange={(e) => setSearchQuery(e.target.value)} /></div>
                        <div className="col-6 text-end"><button className="btn-outline rounded" onClick={createProductHandler}><i class="bi bi-pencil-square"></i> Create Product</button></div>
                    </form>
                    {
                        loading ? (
                            ...
                            <>
                                <table className="table rounded border table-hover table-light text-center shadow">
                                    ...
                                    <tbody>
                                        {products.filter(product => product.title.toLowerCase().includes(searchQuery.toLowerCase()) || product.category.toLowerCase().includes(searchQuery.toLowerCase())).map((product) => (
                                            <>
                                                <tr className="" key={product._id}>
                                                    <td><img src={process.env.PUBLIC_URL + `/products/${product.src}`} clasName="img-fluid rounded img-thumbnail" style={{ width: '8vw' }} /></td>
                                                    <td className="align-middle">{product.title}</td>
                                                    <td className="align-middle">$ {product.price}.00</td>
                                                    <td className="align-middle">{product.category}</td>
                                                    <td className="align-middle">
                                                        <button className="btn rounded p-0 m-0" onClick={() => navigate(`/admin/productlist/editproduct/${product._id}`)}><i class="bi bi-pencil-square"></i> </button>
                                                    </td>
                                                    <td className="align-middle">
                                                        <button className="btn rounded p-0 m-0" onClick={() => deleteProductHandler(product)}><i class="bi bi-trash3"></i> </button>
                                                    </td>
                                                </tr>
                                            </>
                                        ))}
